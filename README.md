...
这是一个面向上海理工大学一网畅学（TronClass，`https://1906.usst.edu.cn/`）的浏览器用户脚本。它在课程页和学习活动/课件详情页显示悬浮面板，读取当前活动或整门课程的附件，并使用当前浏览器已经登录的一网畅学 session 获取下载地址。

## 功能

- 自动识别当前课程活动 ID，并适配 TronClass 的 SPA 页面切换。
- 在课程页枚举当前有权访问课程中的活动及其附件，支持下载整门课程的全部附件。
- 显示课程/活动 ID、附件数量、附件名称、所属活动、下载状态和错误信息。
- 支持一个活动中的多个附件。
- 优先请求原始文件，支持 PPT、PPTX、PDF、DOC、DOCX 等常见课件。
- 原文件地址不可用时，尝试平台提供的预览地址。
- 预览接口返回 PDF 时，Office 文件会按 `.pdf` 文件名保存。
- 提供“刷新附件”“下载当前课件”“下载本活动附件”“下载本课程全部附件”按钮。
- 整门课程的活动和附件列表按顺序读取；多个文件也按顺序下载，并在文件之间留出短暂间隔。
...
GET /api/courses/<courseId>/activities?sub_course_id=0
GET /api/uploads/reference/document/<referenceId>/url
GET /api/uploads/reference/document/<referenceId>/url?preview=true
```

整课模式先读取当前课程的 `activities` 列表，再逐个读取活动的 `upload_references`；只处理平台向当前登录账号正常返回的活动和附件，不会尝试访问其他课程。TronClass 不同版本的字段可能略有差异。脚本优先解析实际返回的 `activities`、`references`、`id`、`title`、`name` 等字段；如果单个活动读取失败，会继续扫描其余活动，并显示部分失败提示。

预览接口如果仍处于生成中、返回空地址、返回 HTML 在线查看器而不是 PDF，脚本无法在浏览器端可靠地把在线查看器转换成 PDF；此时会显示失败原因，不会静默伪造文件。

## 故障排查

- **活动 ID 未识别**：在课程章节页仍可使用“下载本课程全部附件”；若只下载当前活动，请进入该活动详情页并点击“刷新附件”。
- **401**：重新登录一网畅学。
- **403**：当前账号或当前活动没有访问权限。
